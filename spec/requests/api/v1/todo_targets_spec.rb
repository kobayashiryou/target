...
RSpec.describe "TodoTarget", type: :request do
  describe "GET api/v1/todo_targets" do
    ...
    context "companyがログインしている時" do
      it "todo_target一覧が全て表示される" do
        sign_in company
        subject
        expect(response.body).to include "#{targets[0].body}" && "#{a_targets[0].body}"
    ...
    context "departmentがログインしている時" do
      it "ログインしているdepartmentのtodo_target一覧が表示される" do
        sign_in department
        subject
        expect(response.body).to include "#{targets[0].body}"
        expect(response.body).to not_include "#{a_targets[0].body}"
        expect(response).to have_http_status(:ok)
      end
    end
