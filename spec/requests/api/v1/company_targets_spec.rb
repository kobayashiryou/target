  ...
  describe "PATCH api/v1/company_targets/:id" do
    ...
    context "指定したcompany_targetを編集した時" do
      it "company_targetは編集されてリダイレクトする" do
        expect { subject }.to change { CompanyTarget.find(company_target_id).monthly_target }.from(company_target.monthly_target).to(params[:company_target][:monthly_target]) &
